 ...
 * @author Dell
 */
public abstract class CiRelUtil {
    /**
     * 作为唯一标识的id
     */
    ...
    public CiRelUtil(String sourceKey, String targetKey) {
        this.sourceKey = sourceKey;
        this.targetKey = targetKey;
    }

    public CiRelUtil(String sourceKey, String targetKey, Integer count) {
        this.sourceKey = sourceKey;
        this.targetKey = targetKey;
    ...
    /**
     * 循环执行的找下级方法(影响分析和根因分析使用)
     *
     * @param ids 要查出的ids
     * @return
     */
    public abstract Object getDateByImpactAnalysisAndRootCauseAnalysis(List<String> ids, String relIds, String ciTypeId, List<String> ciCodes);
     ...
     * @return
     */
    public Map<String, Object> getData(String startCiCode, String endCiCode) {
        List<String> ciCodes = new ArrayList<String>();
        ciCodes.add(startCiCode);
        return getData(ciCodes, endCiCode);
    }
     ...
     * @return
     */
    public Map<String, Object> getDataByImpactAnalysisAndRootCauseAnalysis(String id, String relIds, String ciTypeId, String ciCode) {
        List<String> ids = new ArrayList<String>();
        ids.add(id);
        List<String> ciCodes = new ArrayList<String>();
        ciCodes.add(ciCode);
        return getDataByImpactAnalysisAndRootCauseAnalysis(ids, relIds, ciTypeId, ciCodes);
    }
     ...
     * @return
     */
    public Map<String, Object> getData(List<String> ciCodes, String endCiCode) {
        Map<String, Object> res = new HashMap<String, Object>();
        if (ciCodes != null && ciCodes.size() > 0) {
            getRel(ciCodes, endCiCode);
        }
     ...
     * @return
     */
    public Map<String, Object> getDataByImpactAnalysisAndRootCauseAnalysis(List<String> ids, String relIds, String ciTypeId, List<String> ciCodes) {
        Map<String, Object> res = new HashMap<String, Object>();
        if (ciCodes != null && ciCodes.size() > 0) {
            getRelByImpactAnalysisAndRootCauseAnalysis(ids, relIds, ciTypeId, ciCodes);
        }
        for (int i = 0; i < resList.size(); i++) {
            JSONObject one = resList.getJSONObject(i);
            String sourceCiId = (String) one.get("sourceCiId");
     ...
     */
    private void getRel(List<String> ids, String endCiCode) {
        if (count > 0) {
            count--;
            resIds.addAll(ids);
            ...
            if (obj != null) {
                ...
                if (data != null && data.size() > 0) {
                    for (Object object : data) {
                        ...
                        if (targetObj != null) {
                            String targetBm = targetObj.toString();
                            if (endCiCode.equals(targetBm)) {
                                dge = false;
                            }
                        }
                    }
                ...
                if (data != null && data.size() > 0) {
                    ...
                    for (Object object : data) {
                        ...
                        if (sourceObj != null && targetObj != null) {
                            String sourceBm = sourceObj.toString();
                            String targetBm = targetObj.toString();
                            idTemps.add(sourceBm);
                            idTemps.add(targetBm);
                            resList.add(one);
                        }
                    }
    ...
    /**
     * 获取关联节点和线(影响分析和根因分析使用)
     * @return ids是找到所有节点的id集合， 是找到所有节点的关系集合
     */
    private void getRelByImpactAnalysisAndRootCauseAnalysis(List<String> ids, String relIds, String ciTypeId, List<String> ciCodes) {
        if (count > 0) {
            count--;
            resIds.addAll(ciCodes);
            resIds = resIds.stream().distinct().collect(Collectors.toList());
            Object obj = getDateByImpactAnalysisAndRootCauseAnalysis(ids, relIds, ciTypeId, ciCodes);
            if (obj != null) {
                JSONArray data = JSON.parseArray(JSON.toJSONString(obj));
                if (data != null && data.size() > 0) {
                    List<String> idTemps = new ArrayList<String>();
                    ...
                    resIds.addAll(tempIds2);
                    if (tempIds2 != null && tempIds2.size() > 0) {
                        getRelByImpactAnalysisAndRootCauseAnalysis(tempIds2, relIds, ciTypeId, tempIds2);
                    }
                }
            }
