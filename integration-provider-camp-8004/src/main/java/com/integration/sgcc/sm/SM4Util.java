...
public class SM4Util extends SM4Utils {
    ...
    public static void main(String[] args) {
        ...
        String plainText = "{}";
        /**
         * 获取随机的key，其编码格式为hex编码，在项目的使用中，
         * 将此秘钥进行文件配置，注意：不能使用硬编码方式使用！
         * 若在传输通道中，可以使用非对称算法进行二次加密后再传输；
         */
        String key = "6a0150d920172857599c94bdd0a233a6";
        System.out.println("随机获取的SM4算法ECB模式加解密的秘钥为："  +key );
        String encryptBySM4ECB = sm4Util.encryptBySM4ECB(plainText,key);
        System.out.println("国密SM4算法ECB模式加密后为：" + encryptBySM4ECB);
        String decryptBySM4ECB = sm4Util.decryptBySM4ECB(encryptBySM4ECB,key);
        System.out.println("国密SM4算法ECB模式解密后为：" + decryptBySM4ECB);
        System.out.println("");
